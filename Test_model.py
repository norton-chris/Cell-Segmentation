...
dims = 256
# Predict on patches
model = load_model('h5_files/model-best-unet.h5', # train_UNet512TIF50E-20220519-23.41.h5
...
for path in os.listdir(test):
    ...
    patcher_img = Random_patcher(img, lab, batch_size=1, input_shape=(dims, dims, 1), step=dims)
    images, masks = patcher_img.patch_image()
    print(images.shape)
    preds_test = model.predict(images.reshape((1,dims,dims,1)), verbose=1)
    ...
    for i in range(0, len(preds_test)):
        ...
        # showing image
        plt.imshow(images.reshape((1,dims,dims))[i])
        plt.axis('off')
        plt.title("Image")
        ...
        # showing label
        plt.imshow(masks.reshape((1,dims,dims))[i])
        plt.axis('off')
        plt.title("Label")
        ...
        intersection = np.logical_and(masks.reshape((1,dims,dims))[i], preds_test[i])
        union = np.logical_or(masks.reshape((1,dims,dims))[i], preds_test[i])
        iou_score = np.sum(intersection) / np.sum(union)
        print("IOU:", iou_score)
        ca = calculate_accuracy(masks.reshape((1,dims,dims))[i], preds_test[i])
